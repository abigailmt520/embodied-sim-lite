# -*- coding: utf-8 -*-
"""
run_coverage_matrix.py  ——  RQ4 · 覆盖矩阵：分层+联合框架 vs 4 个公平 baseline
==============================================================================
论文 RQ4：证明分层+联合审计的覆盖面比已有 baseline 更广——尤其 **joint 层非冗余、
是抓双态耦合（场景 B）的唯一手段**。

🔴 诚实纪律：每个 baseline 给**胜任工程师的最佳实现**（不 strawman）；逐格**真实运行验证**
   （不想当然）；若任何 baseline 抓到我们 miss 的，**如实报告**（不藏）。目标=诚实覆盖对比、非全赢。

5 个方法（行）：
  M1 代码/数据完整性（哈希/校验和）—— 现有 RV/attestation 路线。
  M2 物理-only（EC1-EC5+EC5'，无契约/joint）。
  M3 契约-only（C1-C3+CI，无物理/joint）。
  M4 朴素并行（物理+契约并行，**无 joint 交叉核对**）—— 关键对照。
  M5 我们的完整框架（物理+契约+**joint**）。

自欺实例（列）：契约 L-1/L-2、seq冻结、断流谎称在线；物理 P-1(能量)、CF-2(穿透)；
  场景 A(穿墙幽灵)、场景 B(双态耦合)；data-tamper(数据篡改，code-integrity 的本职威胁)。

运行：python audit/run_coverage_matrix.py    （slip=0.30 = CI 可估区，见 Phase3）
"""
...
SLIP = 0.30      # CI 可估区（Phase3）；部署 slip=0.05 下 CI 退化、契约靠 C1，矩阵脚注说明


# ====================================================================
# 数据生成：每个实例 = {truth, odom, ledger, session, data_ok, slip}
# ====================================================================
def _ledger_row(env):
...
def make_instances():
    ...
    for i in range(len(t0), len(t_st)):
        sess_st[i]["recv_t"] = float(i)                      # 墙钟推进、数据冻结、仍 online
    ...
    tc, oc, lc = run_maze(fault={"mode": "CF-2", "skip_pushout": True},
                          start=[37.0, 20.0], steps=40)        # 撞外墙、不修正穿透
    ...
    # —— 数据篡改（code-integrity 的本职威胁；语义审计信任数据→看不见）——
    o_tamper = o0.copy(); o_tamper[20] = o_tamper[20] + np.array([0.002, 0.0])  # 微小事后篡改
    inst["data_tamper"] = dict(truth=t0, odom=o_tamper, ledger=l0, session=_session(t0, o_tamper),
                               data_ok=False)               # 校验和不匹配
    return inst
...
def m_code_integrity(I):
    """M1 代码/数据完整性：校验数据流校验和。data_ok=False（事后篡改）→ 抓；语义自欺→数据自洽→漏。"""
...
def _physics_red(I):
    ...
    ec5p = ec5_prime(I["truth"], WALLS, RADIUS)              # EC5'（物理内真值-vs-地图）
    if not ec5p["ok"]:
        red.append("EC5P")
    return red
...
def m_naive_parallel(I):  return m_physics_only(I) or m_contract_only(I)        # 无 joint
...
COL_SHORT = {"healthy": "健康", "L1_full_leak": "L1全泄漏", "L2_partial_leak": "L2部分泄漏",
             "seq_freeze": "seq冻结", "stall_online": "断流谎在线", "P1_neg_damp": "P1能量",
             "CF2_skip_pushout": "CF2穿透", "scenA_phantom": "场景A幽灵", "scenB_coupling": "场景B耦合",
             "data_tamper": "数据篡改"}


def main():
    print("=" * 100)
    print("  RQ4 · 覆盖矩阵（逐格真实运行验证；slip=0.30 CI 可估区）")
    print("=" * 100)
    inst = make_instances()
    grid = {}
    for cname in COLS:
    ...
    for mname, _ in METHODS:
        ...
        for c in COLS:
            caught = grid[(mname, c)]
            mark = ("⚠误报" if (c == "healthy" and caught) else ("🔴抓" if caught else "·漏"))
            row += f"{mark:<11}"
        print(row)

    # —— 关键结论逐项验证 ——
    print("\n" + "=" * 100)
    print("  RQ4 头条 + 诚实分析（逐格验证结论）")
    print("=" * 100)
    g = grid
    # ① 健康零误报
    fp = [m for m, _ in METHODS if g[(m, "healthy")]]
    print(f"  ① 健康零误报: {'✅ 全方法无误报' if not fp else '⚠️ 误报: '+str(fp)}")
    # ② code-integrity miss 全部语义、唯抓 data_tamper
    sem = [c for c in COLS if c not in ("healthy", "data_tamper")]
    ci_sem = [c for c in sem if g[("M1 代码完整性", c)]]
    print(f"  ② 代码完整性: 抓 data_tamper={g[('M1 代码完整性','data_tamper')]}, miss 全部语义自欺={not ci_sem} "
    ...
    print(f"  ③ 物理-only miss 契约自欺(L1)={phys_miss_contract} | 契约-only miss 物理自欺(P1)={contract_miss_phys} "
          f"{'✅' if (phys_miss_contract and contract_miss_phys) else '⚠️'}")
    # ④ 头条：朴素并行 miss 场景B，唯我们(joint)抓
    naive_miss_B = not g[("M4 朴素并行(无joint)", "scenB_coupling")]
    ours_catch_B = g[("M5 我们(含joint)", "scenB_coupling")]
    print(f"  ④ 🔴头条: 朴素并行 miss 场景B(双态耦合)={naive_miss_B} ∧ 唯我们(joint)抓场景B={ours_catch_B} "
          f"→ {'✅ joint 非冗余、必要' if (naive_miss_B and ours_catch_B) else '❌ 见矩阵'}")
    # ⑤ 诚实非全赢：data_tamper 唯 code-integrity 抓、我们 miss
    ours_miss_tamper = not g[("M5 我们(含joint)", "data_tamper")]
    print(f"  ⑤ 诚实(非全赢): data_tamper 唯 code-integrity 抓、**我们 miss**={ours_miss_tamper} "
          f"(信任根盲区，互补非被支配) {'✅ 如实报告' if ours_miss_tamper else ''}")
...
def make_figure(grid):
    ...
    for i, m in enumerate(rows):
        for j, c in enumerate(COLS):
            caught = grid[(m, c)]
            if c == "healthy":
                col = "#fa0" if caught else "#d7f3e3"; txt = "误报" if caught else "✓"; tc = "#8a1c1c" if caught else "#2c7"
            else:
                col = "#d7f3e3" if caught else "#fbd9d9"; txt = "抓" if caught else "漏"; tc = "#11623b" if caught else "#8a1c1c"
    ...
    ax.set_yticks([i + 0.5 for i in range(len(rows))]); ax.set_yticklabels(rows, fontsize=9.5)
    ax.set_title("RQ4 覆盖矩阵：分层+联合框架 vs 4 baseline（绿=抓, 红=漏, 橙=误报）\n"
                 "头条：M4朴素并行 漏 场景B(双态耦合)，唯 M5(含joint) 抓 → joint 非冗余；data_tamper 唯 M1 抓(我们 miss,诚实)",
