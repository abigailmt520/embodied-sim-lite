...
- `publish.sh`：公开仓推送唯一入口——自测 → 待推分支全树审计 → 推送，任一环节非零即停；冻结分支 `paper-sync-v1.1`、`paper2-embodied-simlite` 默认拒推（镜像同步须显式 `--allow-frozen`，仍经审计）。用法 `tools/publish.sh [--tag NAME ...] [--allow-frozen] [branch] [remote ...]`；只推显式指定的 tag，不提供全量 --tags。
