...
#   --tag NAME 只推送指定 tag（可重复），且 NAME 须匹配前缀白名单 paper*-*、p5-*、course-* 或版本 tag v<主>.<次>.<修>[-预发布]（FORGE-004 增），其余拒推（exit 4）；
...
while [ "${1:-}" = "--tag" ] || [ "${1:-}" = "--allow-frozen" ]; do
  if [ "$1" = "--tag" ]; then
    case "$2" in paper*-*|p5-*|course-*|v[0-9]*.[0-9]*.[0-9]*) TAGS="$TAGS refs/tags/$2" ;; *) echo "publish: tag $2 不在前缀白名单（paper*-* / p5-* / course-* / v<主>.<次>.<修>[-预发布]），拒推" >&2; exit 4 ;; esac
    shift 2
