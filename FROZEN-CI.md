...
| 2.5 | golden 对照 | 策略 `strict`（裁定②收敛后）：15 件中 11 件逐字节、4 份 session 容差等价（规则见下），任何机器强制；`READY_CHECK_GOLDEN=report` 仅供调试 |
...
- 归一化规则（manifest `entries[].outputs[].normalize` / `compare`）：`audit/eval_summary.json` 去 `generated_at` 后 `json.dumps(indent=2, ensure_ascii=False)`；**四份 session JSON `json_round` 4 位 ＋ `compare: numeric_tol`（浮点 |Δ|≤1e-4 视为相等；结构/键/整数/布尔/字符串逐字节精确）**；**两张 PNG `png_canonical`（PIL 解码→RGBA→固定参数重编码、去元数据）**；stdout 中树根（含 realpath 形）替换为 `<ROOT>`、临时目录替换为 `<TMP>`；其余 9 件零归一化。所有归一化施加在比较层，**course tag 旧入口本身逐字节不动**（A/B 门仍对原始产物成立）。
...
- **两张 PNG**：CI 实测像素 0 差（1430×546 与 1170×650，RGBA 逐像素相同），差异只在 PNG 编码字节（36169 vs 41134、60491 vs 67642）→ 规范化重编码后哈希相同，**逐字节收敛，非豁免**。
...
## 4. golden 基准输出集清单（录制 2026-09-08，`course-2026A` @ bc8fa50，基准机 Mac14,9；15 件，归一化后 sha256；**本表为第四次录制（裁定②第二针：舍 4 位＋容差、PNG 规范化）**，历次见 RECORD.json history）

| 文件 | sha256 |
|---|---|
| audit/eval_episodes.csv | `31b430433291fde94a4f4d56d06a14eaf766daa222656e6545ebc7328cf5666c` |
| audit/eval_metrics.png（png_canonical） | `e4fae15b386160b56648c92b2480335d9a30b3c6be397118cc29dc518babf1db` |
...
| diagnostics/fork_error_curve.png（png_canonical） | `c5ba4c7d5edf4dd124ccbd5b5675d6d3b8c2da1e6d47ccfd0240d3995cd6ead8` |
