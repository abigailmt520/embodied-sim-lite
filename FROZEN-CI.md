...
- 2026-09-15 **FORGE-004 收口 · v1.1.1（CSO 裁定：任务六卡 B 与配套件进公开仓，作冻结基线补丁版）**：
  - **卡 B 关闭态等价（基准机 A/B 探针）**：未设 `--audit-exp`／`AUDIT_EXP` 时，`GET /`、`GET /health`、`GET /openapi.json` 响应字节，实验模式端点路径均 404 且字节同，启动与退出 stdout（应用打印逐字节；uvicorn 访问日志归一客户端端口后逐字节），stderr（归一进程号与端口后），WS 下行（结构与紧凑序列化一致；带 `?exp=1` 亦无握手）——`course-2026A`、冻结提交 `41858c2`、v1.1.1 提交三方一致。
  - **开启态**：`tools/audit_tools/exp_selftest.py` 离线红测全过；`tools/audit_tools/simulate_sessions.py --verify` 端到端 5 会话全过（含起爆前导出、起爆前判定两个边界场景与错票据拒绝）。合入前经独立只读复核，阻断 1 项与主要 3 项均已修复并有用例（会话票据授权、上传与计算限额并移出事件循环、日志即开即关、起爆前结束会话标 incomplete）。
  - **审计包规范 v1.0.1**：`audit_chain.derive_summary` 补两处边界口径（`incomplete`、判定早于起爆），发生在任何数据采集之前；`verify_pack.CHECK_FIELDS` 与 `analyze_packs` 同步。门1–门3 不受影响（golden 三入口不导入实验模式代码）。
...
- **记录**：（发生 hotfix 时于此追加）
- 2026-09-15 **`v1.1.1`**（附注 tag，打在本记录所在提交上）：冻结基线补丁版（**CSO 裁定的一次性例外**：含默认关闭的新增，不属上文 hotfix 定义；tag 名按裁定取版本号，不用 `-hotfix.N`）——审计包规范 v1.0.1、实验模式入口（默认关闭）与随卡两脚本、`CHANGELOG.md` 与两份协作文档、`v*` 版本 tag 发布设施（`publish.sh` 白名单与 `mirror_check.sh` 巡检）。**公开仓自 v1.1.1 起进入冻结基线维护态**，此后只收 hotfix。`course-2026B-pre1` 保持在冻结提交 `41858c2`（tag 永不移动）；它与 v1.1.1 的差异只含默认关闭的新增与文档，课程入口默认行为逐字节一致（门2 A/B）。
- 2026-09-15 **`v1.1.1-hotfix.1`**（附注 tag，打在本记录所在提交上；FORGE-009 冻结基线维护 hotfix 批次一）：①`v1.0.0` 补推 GitHub 镜像（此前只在 Gitee，见下文巡检豁免）；②GitHub tag ruleset 保护范围由 `course-*`／`paper*-*` 扩到 `refs/tags/v*`；③`mirror-check` 工作流加推送时序重试窗口。**不含任何行为代码改动**；`course-2026B-pre1` 与四枚冻结 tag 位置不动。

### 7.1 补丁 tag 命名依据与例外记录（FORGE-009，2026-09-15）

- **常规**：公开仓进入冻结基线维护态后，每次维护打 `<被修复的冻结 tag>-hotfix.<N>`（§7 流程⑤），N 自 1 起、按被修复的基线各自计数。本批次修的是 `v1.1.1` 这条基线（`v1.0.0` 镜像、ruleset 覆盖面、巡检时序），故取 **`v1.1.1-hotfix.1`**。
- **例外（一次性，已发生）**：`v1.1.1` 本身用发布线补丁号而非 `-hotfix.N`——它是冻结基线的**整体发布**（含默认关闭的新增与两份协作文档），由 CSO 裁定按版本号命名，记于上条。
- **两种命名并存的边界**：整体发布用版本号，**仅此一次，不再新增**；此后公开仓的一切维护一律 `-hotfix.N`。判据＝改动是否构成一次基线整体发布；只要是维护性修补（含 CI 配置、镜像补齐、文档订正），一律 `-hotfix.N`。

### 7.2 推送顺序与巡检时序（FORGE-009，2026-09-15）

- **根因**：`tools/publish.sh` 按 `git remote` 输出顺序推送，`github` 排在 `origin`（Gitee）前。master 推送触发的 `mirror-check` 可能在 tag 已到 GitHub、尚未推到 Gitee 的数秒窗口内运行，报 `ONLY-GITHUB` 漂移——属**推送时序**，非漂移（FORGE-004 收口时发生一次，双推完成后重跑即绿）。
- **做法**：**不改 `publish.sh`**（维护态可改动面不含 `tools/`），改在巡检侧加重试窗口——`.github/workflows/mirror.yml` 最多比对 3 次、间隔 60 秒，末次仍漂移才判红。窗口只吸收推送时序；真漂移不会自愈，仍然判红。
- **巡检豁免的现状**：`tools/mirror_check.sh` 中 `refs/tags/v1.0.0` 的挂账豁免自本批次起**自然失效**（两远端已同哈希，比对为 SAME，豁免分支不触发）。豁免行本身保留不删——删行须动 `tools/`，不在可改动面；如需清理另令。
- **不做（CSO 09-15 裁定）**：实验模式离线红测**不进公开仓 CI**——公开仓已冻结，不为冻结面加新门；该红测随实验模式在私有产品仓维护。
